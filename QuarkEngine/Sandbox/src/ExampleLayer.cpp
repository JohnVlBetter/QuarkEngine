	...
	: Layer("ExampleLayer"), mCameraController(1280.0f / 720.0f)
{
	...
	mFlatColorShader = Quark::Shader::Create("FlatColor", flatColorShaderVertexSrc, flatColorShaderFragmentSrc);

	auto textureShader = mShaderLibrary.Load("assets/shaders/Texture.glsl");

	mTexture = Quark::Texture2D::Create("assets/textures/Checkerboard.png");
	mChernoLogoTexture = Quark::Texture2D::Create("assets/textures/ChernoLogo.png");
...
void ExampleLayer::OnUpdate(Quark::Timestep ts)
{
	// Update
...
void ExampleLayer::OnEvent(Quark::Event& e)
{
