...
namespace Quark {
	...
	namespace Utils {
		...
		static GLenum QuarkFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			QK_CORE_ASSERT(mColorAttachments.size() <= 4,"mColorAttachments out of range!");
	...
	{
		QK_CORE_ASSERT(attachmentIndex < mColorAttachments.size(),"");
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(mColorAttachments[attachmentIndex], 0,
			Utils::QuarkFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}
